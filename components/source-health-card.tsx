...
function SourceDiagnosticRow({ diagnostic: d }: { diagnostic: SourceDiagnostic }) {
  return (
    <div className="flex items-start gap-3 rounded-lg border border-border/60 bg-white p-4">
      ...
      <div className="min-w-0 flex-1 space-y-1">
        ...
        <p className="truncate text-xs text-muted-foreground">{d.endpoint}</p>
...
export function SourceHealthCard({
  ...
  const succeeded = diagnostics.filter((d) => d.status === "ok");
  ...
  return (
    <div className="space-y-3">
      {succeeded.length > 0 ? (
        <CollapsibleSection
          variant="inset"
          title={`Healthy sources (${succeeded.length})`}
          subtitle="Successful fetches and ingested results"
          defaultOpen={false}
        >
          <div className="space-y-3">
            {succeeded.map((d) => (
              <SourceDiagnosticRow key={d.sourceId} diagnostic={d} />
            ))}
          </div>
        </CollapsibleSection>
      ) : null}
      {failed.length > 0 ? (
        <CollapsibleSection
          variant="inset"
          title={`Failed sources (${failed.length})`}
          subtitle="Errors and zero-result fetches"
          defaultOpen={false}
        >
          <div className="space-y-3">
            {failed.map((d) => (
